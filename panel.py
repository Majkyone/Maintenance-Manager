...
async def async_register_panel(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if not hass.data.setdefault("my_integration_panel_registered", False):
        await hass.http.async_register_static_paths([
            StaticPathConfig(API_PATH, web_dir, cache_headers=False)
        ])
        hass.data["my_integration_panel_registered"] = True

        await panel_custom.async_register_panel(
            hass,
            frontend_url_path=PANEL_PATH,
            webcomponent_name="my-integration-panel",
...
async def async_unregister_panel(hass: HomeAssistant):
    ...
    if hass.data.setdefault("my_integration_panel_registered", True):
        hass.data["my_integration_panel_registered"] = False
